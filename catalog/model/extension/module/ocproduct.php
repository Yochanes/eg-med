...
class ModelExtensionModuleOcProduct extends Model {
  ...
  $product_data[$result['product_id']] = $this->getProduct($result['product_id']);
   ...
   return $product_data;
	...
	public function getProductsManufacturer($manufacturer_id){
		$sql = "SELECT p.product_id 
		FROM " . DB_PREFIX . "product p  
		LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) 
		LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id)  
		WHERE p.status = '1' AND p.manufacturer_id = ".$manufacturer_id." AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'
