			...
			$recordData = $this->model_extension_module_socnetauth2->getRecord( $this->request->cookie['od_state'] ) )
		{
			...
			if( !empty($data['access_token']) )
			{
				...
				if (isset($arr)&&isset($arr['uid'])&&isset($arr['first_name'])&&isset($arr['last_name'])&&isset($arr['email'])) {
					...
					if( $IS_DEBUG ) echo "Error! Empty Email or user data is not full. Please send request to api-support@ok.ru to get access type: VALUABLE_ACCESS, GET_EMAIL for your API-application. See instruction: https://opencart3x.ru/manual/ok.html<hr>";
				}
