...
class ControllerAccountSocnetauth2 extends Controller {
	...
	public function frame()
	{
			...
			$data = $this->validate($socnetauth_data['data'])) 
		{
			...
			$this->response->redirect( $this->url->link('account/socnetauth2/success', '', true) );
		}
		
		$this->data['action'] = $this->url->link('account/socnetauth2/frame', '', true);
	...
	public function showcode($ar=array())
	{
		...
		$this->session->data['socnetauth2_lastlink'] = $http.$this->request->server['HTTP_HOST'].$this->request->server['REQUEST_URI'];
		...
		$socnetauth2_code = str_replace("#domain#", 
								urlencode( preg_replace("/\/$/", "", $http.$this->request->server['HTTP_HOST']) ), 
								$socnetauth2_code 
								);
	...
	public function confirmform($data = array() )
	{
		...
		$this->data['action'] = $this->url->link('account/socnetauth2/confirmation', '', true);
		...
		$this->response->setOutput($this->load->view('account/socnetauth2_frame_confirmform', $this->data));
